...
void Allocate_vectors(int **local_x_pp, int **local_y_pp, int **local_z_pp,
                      int local_n, MPI_Comm comm);
void Allocate_vectors_for_life(int **local_x_pp, int local_n, int n,
                               MPI_Comm comm);
void Read_vector(int local_a[], int *counts, int *displs, int n,
                 char vec_name[], int my_rank, MPI_Comm comm);
void Print_vector(int local_b[], int *counts, int *displs, int n, char title[],
                  int my_rank, MPI_Comm comm);
void Parallel_vector_sum(int local_x[], int local_y[], int local_z[],
                         int local_n);
void Print_local_vector(int local_b[], int *counts, char title[], int my_rank,
                        MPI_Comm comm);
void compute_local(int local_x[], int n, int counts[], int my_rank, int comm_sz,
                   MPI_Comm comm);
...
void printarray(int **a, int row, int col, int k) {
    ...
    for (i = 1; i < row + 1; i++) {
        for (j = 1; j < col + 1; j++) printf("%d ", a[i][j]);
        printf("\n");
    }
    printf("\n");
}
...
void print_life(int **life, int nRowsGhost, int nColsGhost, int my_rank,
                int local_n) {
    int i, j;
    printf("\n----life print----\n");
    ...
    for (i = 1; i < nRows + 1; i++) {
        for (j = 1; j < nCols + 1; j++) {
            ...
            value = life[i - 1][j - 1] + life[i - 1][j] + life[i - 1][j + 1] +
                    life[i][j - 1] + life[i][j + 1] + life[i + 1][j - 1] +
 ...
 */
void Allocate_vectors_for_life(int **local_x_pp /* out */,
                               int local_n /* in  */, int n /* in*/,
                               MPI_Comm comm /* in  */) {
    int local_ok = 1;
 ...
 * Errors:    One or more of the calls to malloc fails
 */
void Allocate_vectors(int **local_x_pp /* out */, int **local_y_pp /* out */,
                      int **local_z_pp /* out */, int local_n /* in  */,
                      MPI_Comm comm /* in  */) {
    int local_ok = 1;
    char *fname = "Allocate_vectors";
 ...
 */
void Read_vector(int local_a[] /* out */, int counts[] /* in  */,
                 int displs[] /* in  */, int n /* in  */,
                 char vec_name[] /* in  */, int my_rank /* in  */,
                 MPI_Comm comm /* in  */) {
    int *a = NULL;
    int i, local_n;
    int local_ok = 1;
...
void compute_local(int local_x[] /* in */, int n, int counts[] /* in  */,
                   int my_rank /* in */, int comm_sz /* in */,
                   MPI_Comm comm /* in */) {
    // int* b = NULL;
    int i, j, local_n;
    // int nCols=5;
    ...
    // printf("\n ---This is here n=%d rank=%d, local_n=%d nRowsGhost=%d
    // nColsGhost=%d
    ...
    for (k = 0; k < NTIMES && flag != 0; k++) {
        ...
        if ((my_rank % 2) == 0) {
            ...
            MPI_Sendrecv(&(life[nRows][0]), nColsGhost, MPI_INT, upper_rank, 0,
                         &(life[nRows + 1][0]), nColsGhost, MPI_INT, upper_rank,
                         0, comm, &status);

        } else {
            /* exchange down */
            MPI_Sendrecv(&(life[1][0]), nColsGhost, MPI_INT, down_rank, 0,
                         &(life[0][0]), nColsGhost, MPI_INT, down_rank, 0, comm,
                         &status);
        }
        ...
        if ((my_rank % 2) == 1) {
            ...
            MPI_Sendrecv(&(life[1][0]), nColsGhost, MPI_INT, down_rank, 1,
                         &(life[0][0]), nColsGhost, MPI_INT, down_rank, 1, comm,
                         &status);
        }
        ...
        // Each MPI process sends its rank to reduction, root MPI process
        // collects the result
        int reduction_flag = 0;
        MPI_Allreduce(&flag, &reduction_flag, 1, MPI_INT, MPI_SUM,
                      MPI_COMM_WORLD);
